# Derived from REPOLLO, originally developed by Sara Porras Bedmar.
# Original work Copyright (c) 2026, saraporrasbedmar.
#
# Modified for the Auriga + CLUMPY pipeline by Júlia Gouvêa Mamprim.
#
# The upstream code is distributed under the BSD 3-Clause License.
# The original license notice, conditions, and disclaimer are preserved in:
...
class RepopAlgorithm:
    ...
    def _as_value(self, array, unit=None):
        """
        Return plain NumPy values from either an Astropy Quantity or an array.
        """
        if hasattr(array, "to") and unit is not None:
            return array.to(unit).value
        if hasattr(array, "value"):
            return array.value
        return np.asarray(array)

    def _rho_s_from_vmax_rs(self, Vmax, r_s):
        """
        NFW scale density from Vmax and r_s.

        Returns rho_s in Msun/kpc^3.
        """
        Vmax_val = self._as_value(Vmax, u.km / u.s)
        r_s_val = self._as_value(r_s, u.kpc)

        cosmo_G = self.input_dict["cosmo_constants"]["G"]
        G_val = cosmo_G.to(u.kpc * u.km**2 / (u.Msun * u.s**2)).value

        x_rmax = self.RmaxoverrS("NFW")
        f_rmax = np.log(1.0 + x_rmax) - x_rmax / (1.0 + x_rmax)

        return (Vmax_val**2 * x_rmax) / (
            4.0 * np.pi * G_val * r_s_val**2 * f_rmax
        )
    ...
    def _concatenate_catalog_chunks(self, chunks):
        """
        Concatenate the per-bin dictionaries produced during one iteration.
        """
        catalog = {}
        ...
        for key, parts in chunks.items():
            ...
            try:
                catalog[key] = np.concatenate(parts)
            except Exception:
                # Fallback for Astropy Quantity lists in older versions.
                if hasattr(parts[0], "unit"):
                    unit = parts[0].unit
                    catalog[key] = (
                        np.concatenate([p.to(unit).value for p in parts])
                        * unit
                    )
                else:
                    raise
    ...
    def _build_table_catalog(self, catalog):
        """
        Build the HDF5 table compatible with the previous TXT output.

        The output table has columns:
            Js, D_Earth, Vmax, theta_s, Cv, r_s, rho_s,
            Xearth, Yearth, Zearth
        """
        required = [
            "Js", "D_GC", "D_Earth", "Vmax", "theta_s", "Cv", "R_s",
            "galactocentric_X", "galactocentric_Y", "galactocentric_Z",
            "engulfs_Earth",
        ]
        ...
        if missing:
            raise KeyError(
                "Cannot build table catalog. Missing datasets: "
                + ", ".join(missing)
            )

        Js = self._as_value(catalog["Js"], u.Unit("GeV2 / cm5"))
        Dearth = self._as_value(catalog["D_Earth"], u.kpc)
        Vmax = self._as_value(catalog["Vmax"], u.km / u.s)
        theta_s = self._as_value(catalog["theta_s"], u.deg)
        Cv = self._as_value(catalog["Cv"], u.dimensionless_unscaled)
        r_s = self._as_value(catalog["R_s"], u.kpc)

        rho_s = self._rho_s_from_vmax_rs(catalog["Vmax"], catalog["R_s"])

        Xgc = self._as_value(catalog["galactocentric_X"], u.kpc)
        Ygc = self._as_value(catalog["galactocentric_Y"], u.kpc)
        Zgc = self._as_value(catalog["galactocentric_Z"], u.kpc)

        position_Earth = self.input_dict["host"]["position_Earth"].to(u.kpc).value

        # Earth-centered coordinates, following the convention used in the
        # previous code: +X points towards the Galactic Center.
        Xearth = -(Xgc - position_Earth[0])
        Yearth = Ygc - position_Earth[1]
        Zearth = Zgc - position_Earth[2]

        engulfs_Earth = np.asarray(catalog["engulfs_Earth"], dtype=bool)

        if "survives_Roche" in catalog:
            survives_Roche = np.asarray(catalog["survives_Roche"], dtype=bool)
        else:
            survives_Roche = np.ones_like(engulfs_Earth, dtype=bool)

        n_generated = int(len(Js))
        n_removed_engulfing = int(np.sum(engulfs_Earth))
        n_removed_roche = int(np.sum((~survives_Roche) & (~engulfs_Earth)))

        # Agreed behavior:
        #   - do not write Earth-engulfing halos
        #   - do not write Roche-failed halos
        keep = (~engulfs_Earth) & survives_Roche

        data = np.column_stack((
            Js, Dearth, Vmax, theta_s, Cv, r_s, rho_s,
            Xearth, Yearth, Zearth
        ))
        ...
        stats = {
            ...
            "n_removed_engulfing": n_removed_engulfing,
            "n_removed_roche": n_removed_roche,
            "n_saved": int(data.shape[0]),
        }

        return data, halo_names, stats

    def _write_table_iteration(self, h5_file, iter_idx, catalog):
        """
        Write one iteration as a table-like HDF5 group.
        """
        if f"iteration_{iter_idx}" in h5_file:
            del h5_file[f"iteration_{iter_idx}"]

        iter_group = h5_file.create_group(f"iteration_{iter_idx}")

        data, halo_names, stats = self._build_table_catalog(catalog)

        data_dset = iter_group.create_dataset(
            "data",
            data=data,
            chunks=True,
            compression="gzip",
        ...
        print(
            f"    iteration {iter_idx}: saved {stats['n_saved']} halos "
            f"(removed engulfing={stats['n_removed_engulfing']}, "
            f"removed Roche={stats['n_removed_roche']})"
        )

    def interior_full_repop(self):
        """
        Full-repopulation loop.
        ...
        outpath = self.path_output + "fullrepop_" + self.configuration + ".h5"

        # Use write mode to avoid mixing old and new output layouts if the file
        # already exists from a previous run.
        with h5py.File(outpath, "w") as f:
            input_group = f.create_group("inputs")
            aaa = copy.deepcopy(self.input_dict_strings)
            aaa["configurations"] = aaa["configurations"][self.configuration]
            self.store_dict_as_hdf(input_group, aaa)

            for iter_idx in range(self._its):

                if self._prntfrq is not None and iter_idx % self._prntfrq == 0:
                    print("    %s %s: it %d" % (
                    ...
                    progress = open(
                        self.path_output + "progress_"
                        + self.configuration + ".txt",
                        "a",
                    )
                    progress.write(
                        self.configuration + ", iteration " + str(iter_idx)
                    )
                    progress.write(
                        "        %.3f  %s\n" %
                        (
                ...
                # Calculate the subhalo population in bins to control memory.
                ...
                while self._m_min < self.RangeMax:
                    ...
                    # calculate_characteristics_subhalo may return early for an
                    # empty bin, leaving subhalo_data empty.
                    if len(self.subhalo_data) == 0:
                        self._m_min = self._m_max
                        continue

                    for key, array in self.subhalo_data.items():
                        chunks.setdefault(key, []).append(array.copy())

                    self.logging_info(
                        "All characterizations collected, loop continues."
                    )

                    self._m_min = self._m_max

                catalog = self._concatenate_catalog_chunks(chunks)

                if len(catalog) == 0:
                    catalog = {
                        "Js": np.array([]) * u.Unit("GeV2 / cm5"),
                        "D_GC": np.array([]) * u.kpc,
                        "D_Earth": np.array([]) * u.kpc,
                        "Vmax": np.array([]) * (u.km / u.s),
                        "theta_s": np.array([]) * u.deg,
                        "Cv": np.array([]) * u.dimensionless_unscaled,
                        "R_s": np.array([]) * u.kpc,
                        "galactocentric_X": np.array([]) * u.kpc,
                        "galactocentric_Y": np.array([]) * u.kpc,
                        "galactocentric_Z": np.array([]) * u.kpc,
                        "engulfs_Earth": np.array([], dtype=bool),
                        "survives_Roche": np.array([], dtype=bool),
                    }

                self._write_table_iteration(f, iter_idx, catalog)
                f.flush()
