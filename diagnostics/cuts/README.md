...
The full 500-repopulation scan for both disruption scenarios can be
launched with:
...
scenario. The dotted vertical lines mark the relative `Js` below
which 99% of the subhalos lie in the median realization. The 50%, 90%,
...
The plotter is also compatible with the legacy
`relative_js_cumulative_*_cache.npz` files generated by the original
Lattes script.
